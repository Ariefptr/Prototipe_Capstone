define larasati = Character("Larasati",color ="#c9166d")
define sriyani = Character("Sriyani",color = "#296a38")
define rani = Character("Rani",color = "#d07e13ff")
<<<<<<< HEAD
define makara = Character("Makara",color = "#000000")
=======
define ranisweater = Character("Rani",color = "#16b9cfff")
define makhluk_1 = Character("Makara",color = "#000000")
define satria = Character("Satria",color = "#2d299e")
>>>>>>> e7a048253ba2ce847fc6eefe69690b8d72d52ec4
define makhluk = Character("???",color = "#000000")
define narrator = Character("   ",color = "#000000")

image bg envi mom = im.Scale("envi_mom.jpg",1280,720)
image bg desa_kawung = im.Scale("desakawung.png",1280,720)
image bg bukit = im.Scale("bukit.png",1280,720)
image bg kamar_larasatimalam = im.Scale("kamar_larasati21.png",1280,720)
image bg ruang_menjahit = im.Scale("ruang_menjahit.png",1280,720)
image bg kamar_larasati = im.Scale("kamar_larasati.png",1280,720)
<<<<<<< HEAD
image bg cafe = im.Scale("Cafe.png",1280,720)
=======
image phone = im.Scale("phone.png",1280,720)
>>>>>>> e7a048253ba2ce847fc6eefe69690b8d72d52ec4
image lolilaras_base = im.Scale("larasatilolibase.png",442,539)
image lolilaras_bahagia = im.Scale("larasatilolibahagia.png",442,539)
image lolilaras_penasaran = im.Scale("larasatilolipenasaran.png",442,539)
image lolilaras_senang = im.Scale("larasatilolisenang.png",442,539)
image larasatisma_bingungcemas = im.Scale("larasatismabingungcemas.png",533,620)
image larasatisma_bingungheran = im.Scale("larasatismabingungheran.png",533,620)
image larasatisma_kagetbiasa = im.Scale("larasatismakagetbiasa.png",533,620)
image larasatisma_kagetemoji = im.Scale("larasatismakagetemoji.png",533,620)
image larasatisma_kagetemoji2 = im.Scale("larasatismakagetemoji2.png",533,620)
image larasatisma_kagethoror = im.Scale("larasatismakagethoror.png",533,620)
image larasatisma_kalem = im.Scale("larasatismakalem.png",533,620)
image larasatisma_murungsedih = im.Scale("larasatismamurungsedih.png",533,620)
image larasatisma_pasrah = im.Scale("larasatismapasrah.png",533,620)
image larasatisma_senang = im.Scale("larasatismasenang.png",533,620)
image larasatisma_senyum = im.Scale("larasatismasenyum.png",533,620)
image larasatisma_tertarik = im.Scale("larasatismatertarik.png",533,620)
image larasati_bingungcemas = im.Scale("casuallarasatibingungcemas.png",380,637)
image larasati_bingungheran = im.Scale("casuallarasatibingungheran.png",380,637)
image larasati_kagetbiasa = im.Scale("casuallarasatikagetbiasa.png",380,637)
image larasati_kagetemoji1 = im.Scale("casuallarasatikagetemoji1.png",380,637)
image larasati_kagetemoji2 = im.Scale("casuallarasatikagetemoji2.png",380,637)
image larasati_kagethoror = im.Scale("casuallarasatikagethorror.png",380,637)
image larasati_kalem = im.Scale("casuallarasatikalem.png",380,637)
image larasati_murungsedih = im.Scale("casuallarasatimurungsedih.png",380,637)
image larasati_pasrah = im.Scale("casuallarasatipasrah.png",380,637)
image larasati_senang = im.Scale("casuallarasatisenang.png",380,637)
image larasati_senyum = im.Scale("casuallarasatisenyum.png",380,637)
image larasati_tertarik = im.Scale("casuallarasatitertarik.png",380,637)
image sriyani_diam = im.Scale("sriyanidiam.png",614,641)
image sriyani_ketawa = im.Scale("sriyaniketawa.png",614,641)
image sriyani_ketawav2 = im.Scale("sriyaniketawav2.png",614,641)
image sriyani_senang = im.Scale("sriyanisenang.png",614,641)
image sriyani_senyum = im.Scale("sriyanisenyum.png",614,641)
image sriyanitua_bingung = im.Scale("sriyanituabingung.png",612,674)
image sriyanitua_pasrah = im.Scale("sriyanituapasrah.png",612,674)
image sriyanitua_senang = im.Scale("sriyanituasenang.png",612,674)
image sriyanitua_senyum = im.Scale("sriyanituasenyum.png",612,674)
image sriyanitua_bahagia = im.Scale("sriyanituasenyumbahagia.png",612,674)
image sriyanitua_tulus = im.Scale("sriyanituasenyumtulus.png",612,674)
image sriyanitua_serius = im.Scale("sriyanituaserius.png",612,674)
image rani_base = im.Scale("ranibase.png",540,667)
image rani_jahil = im.Scale("ranijahil.png",540,667)
image rani_ngodain = im.Scale("raningodain.png",540,667)
image rani_senang = im.Scale("ranisenang.png",540,667)
image rani_senyum = im.Scale("ranisenyum.png",540,667)
image rani_seyumciptadent = im.Scale("ranisenyumciptadent.png",540,667)
image ranisweater_base = im.Scale("sweaterranibase.png",380,637)
image ranisweater_nyengir = im.Scale("sweaterraninyengir.png",380,637)
image ranisweater_senang = im.Scale("sweaterranisenang.png",380,637)
image ranisweater_senyum = im.Scale("sweaterranisenyum.png",380,637)
image ranisweater_sus = im.Scale("sweaterranisus.png",380,637)
image ranisweater_usil = im.Scale("sweaterraniusil.png",380,637)
image makara_base = im.Scale("makarabase.png",514,706)
image makara_senang = im.Scale("makarasenang.png",514,706)
image makara_kaget = im.Scale("makarakaget.png",514,706)
image satria_senang = im.Scale("satriasenang.png",510,667)
image satria_senyum = im.Scale("satriasenyum.png",510,667)
image satria_merem = im.Scale("satriasenyummerem.png",510,667)
image satria_serius = im.Scale("satriaserius.png",510,667)
image larasati_casuall_senang = im.Scale("casuallarasatisenang.png",380,640)
image larasati_casuall_senyum = im.Scale("casuallarasatisenyum.png",380,640)
image larasati_casuall_tertarik = im.Scale("casuallarasatitertarik.png",380,640)
image larasati_casuall_pasrah = im.Scale("casuallarasatipasrah.png",380,640)
image larasati_casuall_murungs = im.Scale("casuallarasatimurungsedih.png",380,640)




define narator_text=[
    "Larasati baru saja bangun pagi di hari libur sekolah, Hari libur merupakan hari yang sangat disukai oleh anak sekolah, Karena tidak ada kegiatan yang wajib diikuti, Melainkan hanya dirumah dan bebas untuk melakukan apapun",
    "Larasati bangun dari kasurnya dan menemui Ibunya,Seperti biasa, Ibunya berada di ruang kerja,Mendesain motif batik dan membuat pakaian batik pesanan orang lain,Saat ini Ibu Larasati sedang membuat selendang batik",
    "Dengan mata yang masih kantuk, Larasati melihat Ibunya membuat selendang",
    "Setelah selesai makan, Larasati kembali melihat Ibunya yang sedang memotong kain",
    "Larasati mengambil sisa potongan kain yang terjatuh ke lantai, dan memperhatikan kain tersebut. Di kain tersebut terdapat pola yang Ia tidak ketahui",
    "Larasati mengambil potongan kain-kain batik yang tidak digunakan lalu melihat perbedaan motif dari potongan kain batik tersebut",
    "Sriyani menoleh ke arah potongan batik yang dipegang Larasati",
    "Larasati memperhatikan kembali motif-motif batik yang ada di sekitarnya",
    "Ia kembali ke kamarnya, mengambil buku gambar serta pensil dan penghapus",
    "Mulai perlahan Larasati membuat motif batik dari potongan-potongan kain yang tidak terpakai",
    "Setelah beberapa saat",
    "Larasati menunjukkan hasil gambar motif batik yang Ia buat",
    "Sejak saat itu Larasati suka sekali menggambar motif-motif batik yang ada di ruang kerja Ibunya",
    "Beberapa hari kemudian",
    "Larasati memainkan gulungan-gulungan benang dan memperhatikan ada apa saja isi dari kotak yang biasa disimpan Ibunya untuk menjahit",
    "Lalu Larasati mencoba untuk memasukkan benang ke jarum dan mengikat benang di kancing yang tidak terpakai",
    "Sriyani yang melihat tingkah Larasati hanya tersenyum",
    "Sriyani merasa bangga dari apa yang sudah dilakukan Larasati",
    "Sriyani mengambil kain dari lemari dan membawanya",
    "Larasati sangat senang karena bisa belajar apa yang dia inginkan,sejak saat itu Larasati belajar menjahit untuk pertama kalinya ",
]

# Prolog Larasati masih SD
define text_prolog_larasati=[
    "Hooam... jam berapa ini ?",
    "Delapan lewat ?",
    "Hmmm...hmmm....",
    "Ibu membuat apa saja hari ini ?",
    "Hmmm......",
    "Kenapa kainnya dipotong bu ?",
    "Ibu ini gambar apa ? ",
    "Batik ? kok gambarnya berbeda dari yang ibu buat kemarin",
    "Motif...",
    "Yang ini bagus , eh tapi yang ini juga bagus bu",
    "Ibu kenapa yang ini sedikit berbeda dari motif lainnya ?",
    "Ibu lihat",
    "Ibu lagi buat apa ?", #12
    "Aku pernah melihat nenek melakukan hal yang sama seperti ibu",
    "Aku lupa namanya apa ...",
    "Ah... iya menjahit",
    "Dulu ibu menjahit selendang juga ?", #16
    "Lalu Ibu belajar desain motif batik dari siapa... ?", #17
    "Berarti dulu Ibu belajar menjahit dari nenek ya ?", #18
    "Aku juga ingin belajar menjahit bu...", #19
    "Tapi aku lebih suka menggambar motif-motif batik", #20
    "Hari ini aku hanya menggambar satu motif saja", #21
    "Dari kemeja yang sering ayah pakai saat ada acara keluarga", #22
    "Aku suka....", #23
    "Aku suka yang motif nya berbentuk seperti bunga bu...", #24
    "Walaupun terkadang susah tapi aku bisa dan aku suka....",
    "Belum bu...",
    "Ih... aku mau bu",

]


define text_prolog_sriyani=[
    "Lho kamu sudah bangun ?",
    "Hari ini ibu membuat selendang sama kain",
    "Kamu makan dulu gih",
    "Sudah Ibu buatkan Gudeg",
    "Sepertinya kepanjangan kainnya",
    "Orang yang punya kain ini tidak suka kalau kainnya kepanjangan",
    "Gambar ? Ohh itu motif batik sayang",
    "Batik itu banyak sekali motifnya dan ini bukan disebut gambar melainkan motif",
    "Oh itu motif batik yang ibu buat sendiri desainnya",
    "Hmm...? Wah bagus banget",
    "Ibu hanya menjahit selendang ibu yang sudah lama sayang",
    "Menjahit ...?",
    "Dulu Ibu belajar menjahit selendang dan juga pakaian",
    "Tapi Ibu tidak belajar untuk membuat desain motif batik",
    "Sejak Ibu kenal Ayah",
    "Ayah suka sekali menggambar, terutama motif-motif batik",
    "Dari situ Ibu belajar untuk desain motif batik",
    "Benar, di keluarga Ibu menjahit itu sudah menjadi tradisi keluarga, apalagi menjahit batik",
    "Gimana hasil gambar motif-motif batik kamu hari ini ?",
    "Oh yang itu...",
    "Sayang, kamu suka menggambar motif yang seperti apa ?",
    "Oh iya sayang, kamu udah punya selendang buat acara perpisahan nanti ? ",
    "Kamu mau buat selendang sendiri... ?",
    "Sini kamu lihat ibu dulu, pertama kamu lebarkan dulu kainnya dan lipat seperti ini",
    "berikutnya kamu ukur pakai ini",
]

# chapter 1 = larasati pulang dari sekolah dan menjahit batik

define text_narrator_sma=[
    "Kini Larasati sudah duduk di bangku SMA, dan Dia bukan hanya seorang pelajar",
    "Tapi seorang yang bisa dibilang memiliki bisnis",
    "Desain motif batik dan menjahit, Itulah yang dilakukan Larasati saat ini dan melanjutkan usaha Ibunya",
    "Dan juga sekolah",
    "Larasati menunjuk jarinya ke arah desain motif yang sudah Ia buat",
    "Larasati membuat selendang pesanan orang",
    "Membuat selendang cukup mudah untuknya,karena sudah terbiasa sejak Ia masih anak-anak",
    "Larasati melihat desain motif batik Ibunya yang sudah lama",
    "Ada 3 desain motif batik, tapi Larasati bosan melihat desain yang seperti itu dan ingin membuat desain yang lebih modern",
    "Larasati tidak bisa mengerjakan desain motif batiknya",
    "Karena pikiran yang tidak tenang, ditambah harus belajar untuk ujian dua minggu lagi",
    "Pikiran Larasati benar-benar kacau",
    "Dia tidak bisa berpikir jernih untuk menyelesaikan masalahnya satu per satu",
    "Larasati pun pada akhirnya belum menmenyelesaikan desainnya dan memilih untuk istirahat di kasur",
    "Larasati memejamkan matanya dan tiba-tiba muncul notifikasi dari handphone",
]

define text_larasati_sma=[
    "Aku pulang...",
    "Iya gurunya ada rapat jadi pulang lebih awal",
    "Bukannya aku sudah buat 4 desain kemarin ...?",
    "Bukannya yang ini bagus kalau jadi selendang...?",
    "....",
    "Iya bu...",
    "Selesai",
    "Sekarang saatnya membuat desain motif batik",
    "Apasih padahal desain motif ini bagus loh dan kenapa orangnya juga tidak suka ya ...",
    "Ahh..... malas sekali",
    "Ide apa lagi yang bisa aku buat ....",
    "Ahh... tugas sejarah belum selesai lagi, mana sebentar lagi ujian...",
    "Bagaimana ini ?",
    "Desain motif batik harus selesai dalam beberapa hari kedepan",
    "....",
    "Kurang menarik... ",
    "Yang ini kurang bagus",
    "Ini warnanya.... ",
    "Kalau desain ini dijadikan kemeja kayaknya aneh deh...",
    "Haah....",
    "Kenapa aku jadi tidak yakin begini ya dengan desain yang aku buat",

]

define text_sriyani_sma=[
    "Pulang cepat hari ini kamu ?",
    "Begitu ya sayang, kalau begitu apa kamu bisa melanjutkan desain motif batiknya",
    "Dan juga ini ada 2 kain yang harus kamu jadikan selendang hari ini",
    "Desainnya kurang menarik jika digunakan untuk baju kemeja dan juga tidak cocok untuk kain selendang",
    "Ibu kan sudah bilang ke kamu untuk menggunakan motif yang sudah ibu rekomendasikan",
    "Tidak bisa sayang, orangnya sudah bilang ke Ibu. Dia tidak mau dengan desain motif seperti itu",
    "Kamu ini kenapa sih, kalau kamu tidak ada ide sendiri untuk membuat desain motif batik",
    "Gunakan saja desain yang sudah ibu berikan sayang...",
    "",

]


# larasati dan rani sedang DM lewat handphone

define text_larasati_dm=[
    "Siapa sih nih... Ohh... Rani rupanya",
    "Kosong, memangnya kenapa ?",
    "Ohh yang harganya mahal itu",
    "Mana ada kopi harganya 25 ribu Raniii...",
    "Kenapa gak bahas di WA aja?",
    "Yaudah jam berapa ?",
    "Okelah..."
]

define text_rani=[
    "Sore ini kamu kosong gak ?",
    "Aku mau ngajak kamu ke kafe yang kemarin aku kasih tau",
    "ENGGA MAHAL",
    "Ih, yaudah nanti aku tambahin",
    "Pokoknya nanti datang yahh, aku mau ngomong sesuatu",
    "Nggak asik ah, enakan langsung ketemu sekalian minum kopi",
    "Jam 15:30 kita disana",

]

define text_larasati_kafe=[
    "Ah iya",
    "Dih... macet tahu",
    "Au ah... aku mau pesan kopi dulu",
    "Tempat nya lumaya juga",
    "Tapi lumayan mahal-mahal menunya",
    "Iyaa..",
    "Sudah, tapi aku harus buat ulang lagi",
    "Desain yang aku buat ternyata nggak sesuai keinginan pelanggan",
    "Iyalah",
    "Belum, aku bingung nggak ada ide lagi",
    "Ehh.. udah sih tapi aku nggak yakin hasilnya bagus dimata orang lain",
    "Aku buat yang modelnya lebih modern gitu, tapi rasanya aneh aja kalau diaplikasikan ke kemeja",
    "Ada aku pernah lihat waktu liburan ke Solo",
    "....",
    "Iya sih aku harus membuat desain yang menarik menurut pandangan orang lain",
    "Sudah, aku sudah melihat semua buku gambarnya",
    "Tapi aku cukup bosan melihat model yang begitu-begitu saja",
    "Aku mau buat desain model batikku sendiri",
    "Yaa nanti saja lah , Aku sedang tidak ingin mencari ide untuk desain",
    "Yang mana ? ada banyak tuh",
    "Ohh dia... emang nya kenapa dia ?",
    "Dih... mau deketin anak orang",
    "Emang dia single ?",

]
define text_narrator_kafe=[
    "Pukul 15:35",
    "Rani sudah sampai di kafe tapi Larasati belum datang",
    "Pukul 15:55",
    "Larasati akhirnya datang",
    "Rani melambaikan tangan untuk memberitahu Larasati",
    "Setelah Larasati memesan kopi, mereka melanjutkan pembicaraan",
    "Dan obrolan mereka berlanjut hingga pukul 19:45",
    "Dan mereka pulang ke rumah masing-masing",
]

define text_rani_kafe=[
    "Ternyata belum sampai Dia",
    "Lama banget sih itu anak",
    "Datang juga kamu",
    "Aku sampai pesan kopi duluan",
    "Kenapa lama sekali sih ...?",
    "Dih..... alasan",
    "Iyalah",
    "Yaa.. sesuai standard harga kafe sih",
    "Ngomong-ngomong kemarin kamu lagi bikin desain motif batik ya ?",
    "Udah selesai ?",
    "Lohh...",
    "Ohh.... jadi kamu buat ulang lagi tuh ?",
    "Sudah selesai kan ?",
    "Emang kamu buatnya gimana",
    "Batik tapi modern.. ? Emang ada ya.. ?",
    "Hmm aku nggak tahu banyak sih tentang batik",
    "Tapi kalau desain harusnya tergantung selera masing-masing orang",
    "Kamu tidak menggunakan desain model dari Ayahmu ?",
    "Kamu pernah bilang dulu Ayahmu suka sekali membuat model batik kan?",
    "Memang susah sih kalau membuat desain sendiri",
    "Coba saja kamu cari buku-buku Ayahmu lagi",
    "Siapa tahu ada desain model batik yang dapat membantu kamu",
    "Hmm.. ya tidak apa-apa",
    "Untuk mencari ide memang tidak harus terburu-buru",
    "Eh kamu tahu gak ? kakak kelas kita nggak yang ikut ekskul bahasa Inggris",
    "Itu loh yang namanya .....*bisik-bisik*",
    "Dia masih single tidak ya?",
    "Dih... suka-suka lah",
    "Harusnya sih iya, dia nggak pernah bikin status sama pasangannya sih ",


]


define text_narrator_pulang=[
    "Pukul 20:25",
    "Larasati sampai di rumah", 
    "Dan dia pun teringat harus bersih-bersih dan melanjutkan desainnya",
    "Larasati pergi ke kamarnya",
    "Larasati masih memikirkan desain model batiknya",
    "Ia merasa bahwa desain modelnya masih kurang bagus",
    "Larasati sekali lagi melihat desain model Ibunya",
    "Tapi Larasati tetap penasaran",
    "Ada rasa ingin mencari buku peninggalan Ayahnya",
    "Dan akhirnya",
    "Larasati membuka semua lemari dan laci yang ada di kamarnya dan mencari buku peninggalan ayahnya",
    "Larasati membuka laci yang ada di sebelah kasurnya",
    "Ketika baru dibuka, tiba-tiba ada buku yang tidak pernah Ia lihat sebelumnya",
    "Larasati membuka buku tersebut",
    "Tiba-tiba ada cahaya yang muncul dari buku tersebut",
    "Buku tersebut melayang di hadapan Larasati",
    "Barang-barang berjatuhan ke lantai",
    "Suasana menjadi tegang seperti ada bencana yang datang",
    "Dan Tiba-tiba Larasati masuk ke dalam buku tersebut",
    "Semua menjadi hening",
    "Buku yang Larasati pegang berada di tempat semula",
    "Barang-barang yang berjatuhan kembali ke tempat semula",


]


define text_larasati_pulangkafe=[
    "Aku pulang...",
    "Sudah bu...",
    "Baik bu...",
    "....",
    "Iya bu",
    "Huft...",
    "Mandi gak ya...",
    "Malas mandi lagi",
    "Ah, apa masih ada buku desain motif batik yang belum pernah kubaca ya",
    "Tapi sepertinya sudah aku baca semua sih buku yang Ayah kasih",
    "Sudah lah mandi saja",
    "Ihh.... kenapa aku jadi kepikiran begini",
    "Eh... sebentar. Ini buku apa ya....?",
    "Belum pernah lihat dan siapa yang meletakkannya disini",
    "Hmm....",
    "Rahara Batik. Ini buku punya Ayah ?",
    "Huahh...",
    "Eh bukunya terbang....",
    "Eh kenapa jadi ada gempa begini...???",

]

define text_respon_ibularas=[
    "Baru pulang sayang ?",
    "Sudah makan belum...? Ini lauknya mau ibu masukkin ke dalam lemari es",
    "Oh...yasudah kalau begitu, sebelum tidur mandi dulu ya",
    "Sama desainnya jangan lupa dikerjakan juga ya",
    

]


screen black_screen():
    frame:
        background "#000000"

#screen mengamati1():
#    frame:
        #background "mengamati"

screen white_screen():
    frame:
        background "#ffffff"